...
class GameScreen3 : AppCompatActivity() {
    ...
    suspend fun Timer(winSound:MediaPlayer){
        ...
        while (con) {
            ...
            if (count <= 0) {
                ...
                con = false
                start = false
                var intent = Intent(this@GameScreen3, GameWin::class.java)
    ...
    suspend fun damageToAsteroid(overSound: MediaPlayer) {
        var array = mutableListOf<ImageButton>(ibAst1, ibAst2, ibAst3, ibAst4, ibAst5, ibAst6, ibAst7, ibAst8)
        start = true
        while (start) {
            withContext(Dispatchers.Main) {
            ...
            if (array.all {it.visibility == View.VISIBLE})  {
                ...
                con = false
                start = false
                val intent = Intent(this@GameScreen3, GameOver::class.java)
                startActivity(intent)
            }
        }
    }
